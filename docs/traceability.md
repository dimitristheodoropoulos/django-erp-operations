...
| ERP-REQ-038 | Authentication               | Accounts architecture                       | —             | —           | —                              | `config/settings.py::REST_FRAMEWORK`; `apps/accounts/permissions.py::CustomerAccessPermission` | `tests/api/test_customers.py::test_anonymous_customer_list_requires_authentication; test_anonymous_customer_detail_requires_authentication; test_anonymous_customer_create_requires_authentication` | `python -m pytest -q tests/api/test_customers.py` → 16 passed; `python -m pytest -q` → 35 passed | VERIFIED    |
| ERP-REQ-039 | User Roles                   | Accounts architecture                       | —             | —           | `accounts/0001_create_roles.py` | `apps/accounts/permissions.py::ROLE_ADMIN; ROLE_OPERATIONS; ROLE_READ_ONLY` | `tests/api/test_customers.py::test_admin_can_read_customers; test_admin_can_create_customers; test_operations_can_read_customers; test_operations_can_create_customers; test_read_only_can_read_customers; test_read_only_cannot_create_customers` | `python -m pytest -q tests/api/test_customers.py` → 16 passed; `python -m pytest -q` → 35 passed | VERIFIED    |
| ERP-REQ-040 | Permission Enforcement       | Accounts architecture                       | —             | —           | —                              | `apps/accounts/permissions.py::CustomerAccessPermission`; `apps/api/customer_views.py::CustomerListCreateView; CustomerDetailView` | `tests/api/test_customers.py::test_admin_can_read_customers; test_admin_can_create_customers; test_operations_can_read_customers; test_operations_can_create_customers; test_read_only_can_read_customers; test_read_only_cannot_create_customers` | `python -m pytest -q tests/api/test_customers.py` → 16 passed; `python -m pytest -q` → 35 passed | TESTED      |
